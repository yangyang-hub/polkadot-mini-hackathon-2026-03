...
Phase 8: Polish & Deploy             ← 测试、优化、Polkadot 部署
```
...
- 添加 Polkadot EVM chain 定义
- 降低 `pollingInterval` 至 1000ms (适配 Polkadot EVM 出块时间)
...
### 8.2 Polkadot EVM Deployment

**配置**:
- `packages/foundry/foundry.toml` — 添加 Polkadot EVM RPC
- `packages/nextjs/scaffold.config.ts` — 添加 Polkadot EVM chain
- `.env` — `DEPLOYER_PRIVATE_KEY`, `POLKADOT_RPC_URL`
...
| `packages/nextjs/scaffold.config.ts` | 2 | Polkadot EVM chain + polling |
...
|------|-----------|-------------|--------|
| Quick (Bronze) | 0.01 DOT | 10 | 5 |
| Standard (Silver) | 0.05 DOT | 30 | 10 |
| Epic (Gold) | 0.1 DOT | 50 | 20 |
